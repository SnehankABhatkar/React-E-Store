...
export default function CheckoutStepper() {
  ...
  const [confiramtionToken, setConfiramtionToken] =
  ...
  const handleNext = async () => {
    ...
    if (activeStep === 1) {
      ...
      setConfiramtionToken(stripeResult.confirmationToken);
    }
    if (activeStep === 2) {
  ...
  const confirmPayment = async () => {
    ...
    try {
      ...
      const paymentResult = await stripe?.confirmPayment({
        ...
        confirmParams: {
          confirmation_token: confiramtionToken.id,
        },
      });
    ...
    <Paper sx={{ p: 3, borderRadius: 3 }}>
      ...
      <Box sx={{ mt: 2 }}>
        ...
        <Box sx={{ display: activeStep === 2 ? "block" : "none" }}>
          <Review confirmationToken={confiramtionToken} />
        </Box>
      </Box>
      ...
      <Box display={"flex"} paddingTop={2} justifyContent={"space-between"}>
        ...
        >
          {activeStep === steps.length - 1 && !submitting
